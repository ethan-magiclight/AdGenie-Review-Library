...
const outputPath = path.join(outputDir, "AdGenie能力需求表-v8.xlsx");
const docsPath = path.join(root, "docs", "AdGenie能力需求表-v8.xlsx");
const previewDir = path.join(outputDir, "v8-previews");

const workbook = await SpreadsheetFile.importXlsx(await FileBlob.load(outputPath));
const sheet7 = workbook.worksheets.getItem("7_消费电子商品品类");
sheet7.getRange("F1:AK39").clear({ applyTo: "contents" });
sheet7.getRange("A21:AK39").clear({ applyTo: "contents" });
...
console.log(JSON.stringify({ outputPath, docsPath, formulaErrors: errors.ndjson }, null, 2));
